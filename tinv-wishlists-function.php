		...
		if (0 < count($item_data)) {
			if ($flat) {
				...
				foreach ($item_data as $data) {
					echo esc_html($data['key']) . ': ' . esc_html($data['display']) . '<br>';
				}
...
if (!function_exists('tinv_wishlist_print_meta')) {
	...
	function tinv_wishlist_print_meta($meta = array(), $flat = false)
	{
		...
		$hidden_keys = array();
		if (isset($meta['tinvwl-hidden-fields'])) {
			$hidden_raw = $meta['tinvwl-hidden-fields'];
			unset($meta['tinvwl-hidden-fields']);
			if (is_string($hidden_raw)) {
				$decoded = json_decode($hidden_raw, true);
				if (is_array($decoded)) {
					$hidden_keys = array_map('strval', $decoded);
				}
			} elseif (is_array($hidden_raw)) {
				$hidden_keys = array_map('strval', $hidden_raw);
			}
		}
		foreach ($meta as $key => $value) {
			if (preg_match('/^_/', (string)$key) || in_array((string)$key, $hidden_keys, true)) {
				continue;
			}

			$display = $value;

			if (is_object($display) || is_array($display)) {
				$display = json_encode($display);
			} else {
				$display = wp_strip_all_tags((string)$display, true);
			}

			$item_data[$key] = array(
				'key' => wp_strip_all_tags((string)$key, true),
				'display' => $display,
			);
		}
		...
		foreach ($item_data as $key => $data) {
			if (is_object($data['display']) || is_array($data['display'])) {
				...
				$item_data[$key]['display'] = wp_strip_all_tags((string)$data['display'], true);
			}
		}
		ob_start();
		if ($flat) {
